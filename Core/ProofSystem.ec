...
(* prover *)
module type Prover(O: POracle) ={
  proc prove(s : stm, w : wit) : prf {O.o}
}.

(* verifier *)
module type Verifier(O: POracle) ={
  proc verify(s:stm, p: prf): bool {O.o}
}.

(* proof system *)
module System(R: Relation, P:Prover, V:Verifier, O: POracle) = {
...
section.
  ...
  local lemma corr(s : stm, w : wit) &m:
    exists eps,
      0%r < eps /\
      Pr[Correctness(R, P, V, O).main(s,w) @ &m: res ] >= 1%r - eps by exists 1%r;smt.

  local lemma zero_knowledge &m:
    exists eps, 
      `| Pr[ZK_L(R, P, AZ, O).main() @ &m: res] -
      Pr[ZK_R(R, S, AZ).main() @ &m: res] | <= eps by exists 1%r;smt.
end section.
